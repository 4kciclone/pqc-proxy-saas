...
const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-dark text-slate-200 font-sans selection:bg-primary selection:text-black">
      ...
      <nav className="border-b border-slate-800 bg-dark/80 backdrop-blur-md fixed w-full z-50">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            <a href="#features" className="text-sm font-medium hover:text-primary transition-colors hidden md:block">Technology</a>
            <a href="#how-it-works" className="text-sm font-medium hover:text-primary transition-colors hidden md:block">Integration</a>
            <button 
      ...
      <section className="pt-32 pb-20 px-6">
        <div className="max-w-5xl mx-auto text-center">
          ...
          <p className="text-xl text-slate-400 mb-10 max-w-2xl mx-auto leading-relaxed">
            The "Harvest Now, Decrypt Later" threat is real. PQC Shield provides a drop-in sidecar proxy that upgrades your existing banking & IoT systems to Post-Quantum Cryptography in minutes.
          </p>
          <div className="flex flex-col md:flex-row gap-4 justify-center">
            <button onClick={() => navigate('/sign-in')} className="bg-primary text-black px-8 py-4 rounded-xl font-bold text-lg hover:bg-emerald-400 transition-transform hover:scale-105 shadow-[0_0_30px_rgba(16,185,129,0.3)]">
              Start Free Trial
            </button>
            <button className="px-8 py-4 rounded-xl font-bold text-lg border border-slate-700 hover:bg-slate-800 transition-colors text-white">
              Read Documentation
            </button>
          </div>
        </div>
      ...
      {/* FEATURES GRID */}
      <section id="features" className="py-20 bg-card/50 border-y border-slate-800">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid md:grid-cols-3 gap-12">
            <FeatureCard 
              icon={<Lock className="text-primary h-8 w-8" />}
              title="Quantum-Resistant"
              desc="Uses ML-KEM (Kyber-768) and ML-DSA (Dilithium) to secure handshakes against future quantum computer attacks."
            />
            <FeatureCard 
              icon={<Zap className="text-yellow-400 h-8 w-8" />}
              title="Wire-Speed Latency"
              desc="Hybrid architecture (Go + Rust) ensures <50µs overhead. Your High-Frequency Trading systems won't notice we're there."
            />
            <FeatureCard 
              icon={<Database className="text-accent h-8 w-8" />}
              title="Drop-in Solution"
              desc="No code refactoring required. Deploy our Docker Sidecar in front of your Mainframes or SQL Databases instantly."
            />
          </div>
        </div>
      </section>
      ...
      <section id="how-it-works" className="py-20 px-6">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-3xl font-bold text-white mb-12 text-center">Implementation Architecture</h2>
          <div className="bg-black p-8 rounded-2xl border border-slate-800 font-mono text-sm text-slate-300 shadow-2xl">
            <div className="flex items-center gap-4 mb-6 border-b border-slate-800 pb-4">
              <div className="h-3 w-3 rounded-full bg-red-500"></div>
              <div className="h-3 w-3 rounded-full bg-yellow-500"></div>
              <div className="h-3 w-3 rounded-full bg-green-500"></div>
              <span className="ml-auto text-slate-600">terminal — bash</span>
            </div>
            <p className="mb-4"><span className="text-primary">user@bank-server:~$</span> docker run -d \</p>
            <p className="pl-4 text-accent">--name pqc-sidecar \</p>
            <p className="pl-4 text-accent">-e LICENSE="SAAS-ENTERPRISE-XYZ" \</p>
            <p className="pl-4 text-accent">-p 4433:4433 \</p>
            <p className="pl-4">pqc-shield:latest</p>
            <br/>
            <p className="text-slate-500"># System initializes...</p>
            <p>> 🛡️ PQC Proxy listening on 0.0.0.0:4433</p>
            <p>> 🔐 Established Kyber-768 Tunnel with SaaS Control Plane</p>
            <p>> ✅ Protecting upstream: legacy-mainframe:80</p>
...
const LoginPage = () => (
  <div className="min-h-screen flex flex-col items-center justify-center bg-dark">
    <div className="text-center mb-8">
      ...
      <p className="text-slate-400 mt-2">Sign in to manage your secure nodes</p>
    </div>
    <div className="bg-card p-2 rounded-xl border border-slate-700 shadow-2xl">
...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-dark flex font-sans">
      {/* SIDEBAR */}
      <aside className="w-64 bg-card border-r border-slate-800 flex flex-col fixed h-full">
      ...
      <main className="flex-1 ml-64 overflow-y-auto min-h-screen">
        ...
        <div className="p-8">
          ...
          {activeTab === 'nodes' && (
            <div className="max-w-2xl mx-auto space-y-8">
              <div className="bg-card p-6 rounded-xl border border-slate-800 shadow-lg">
                ...
                <p className="text-slate-400 text-sm mb-6">Update routing logic for deployed proxies instantly.</p>
                <form onSubmit={handleConfigUpdate} className="space-y-4">
                  ...
                  <div>
                    ...
                    <input 
                      type="text" 
                      placeholder="domain.internal:port"
                  ...
                  <button disabled={isUpdating || !selectedProxy} className="w-full bg-primary hover:bg-emerald-400 text-dark font-bold py-3 rounded-lg transition-all flex items-center justify-center gap-2 disabled:opacity-50">
          ...
          {activeTab === 'billing' && (
             <div className="max-w-4xl mx-auto text-center bg-gradient-to-br from-card to-slate-900 border border-slate-700 rounded-2xl p-8">
               <div className="inline-flex p-4 bg-primary/10 rounded-full mb-6"><CreditCard size={32} className="text-primary" /></div>
               <h2 className="text-3xl font-bold text-white mb-2">Enterprise Plan</h2>
               <div className="text-2xl font-bold text-white mb-8">$499<span className="text-sm text-slate-500">/mo per node</span></div>
               <button className="bg-white text-black px-8 py-3 rounded-lg font-bold hover:bg-slate-200 transition-colors">Manage Subscription</button>
               <div className="mt-8 pt-8 border-t border-slate-800 text-left">
                 <h4 className="text-sm font-bold text-slate-300 mb-4">LICENSE KEY</h4>
                 <div className="bg-black p-4 rounded border border-slate-700 font-mono text-primary flex justify-between">
...
const SidebarItem = ({ icon, label, active, onClick }) => (
  <button onClick={onClick} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors text-sm font-medium ${active ? 'bg-primary/10 text-primary border border-primary/20' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
    {icon} {label}
  </button>
);
...
const StatCard = ({ title, value, sub, icon }) => (
  <div className="bg-card p-6 rounded-xl border border-slate-800">
    <div className="flex justify-between items-start mb-4">
      <div><h3 className="text-slate-400 text-xs font-bold uppercase tracking-wider">{title}</h3><div className="text-3xl font-bold text-white mt-1">{value}</div></div>
...
const Table = ({ proxies }) => (
  <table className="w-full text-sm text-left">
    <thead className="text-xs text-slate-500 uppercase bg-slate-900/50">
      <tr><th className="px-6 py-4">Node ID</th><th className="px-6 py-4">Status</th><th className="px-6 py-4">Target</th><th className="px-6 py-4">Uptime</th></tr>
    </thead>
    <tbody className="divide-y divide-slate-800">
      ...
      )) || <tr><td colSpan="4" className="px-6 py-8 text-center text-slate-500">No nodes detected.</td></tr>}
    </tbody>
  </table>
);

// ==========================================
// 4. ROUTER & AUTH CONFIG
// ==========================================
function App() {
  return (
